...
For example, on lines 119, 120, and 121 in the script document I am pulling 3 specific notes from items in our plan. ("Side Screens", "Center Screen", and "Ableton") If you don’t use the same note names we do, these line will cause an error. 
...
The template will display `**PLAN IS UNFINALIZED**` by default. To have a version displayed, you will need to add a plan note titled `Information` with your version name in it (We name our versions acording to the [NATO phonetic alphabet](https://en.wikipedia.org/wiki/NATO_phonetic_alphabet), but any name should work)
